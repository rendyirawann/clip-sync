...
def has_audio(video_path, ffmpeg_path):
    """Check if the video file has an audio stream."""
    try:
        # Resolve ffprobe path based on ffmpeg path
        ffprobe_path = ffmpeg_path.replace("ffmpeg.exe", "ffprobe.exe").replace("ffmpeg", "ffprobe")
        cmd = [ffprobe_path, "-v", "error", "-select_streams", "a", "-show_entries", "stream=codec_type", "-of", "default=nw=1:nk=1", video_path]
        res = subprocess.run(cmd, capture_output=True, text=True, check=True)
        return "audio" in res.stdout.strip()
    except Exception:
        # Fallback to True if ffprobe fails
        return True

def mix_bgm_audio(video_path, bgm_path, ffmpeg_path, duration):
    """Mix original video audio with background Phonk music at low volume."""
    if not bgm_path or not os.path.exists(bgm_path):
        return
    
    print(f"Mixing background music: {bgm_path} (Duration: {duration}s)...")
    temp_output = video_path.replace(".mp4", "_bgm_temp.mp4")
    ...
    try:
        video_has_a = has_audio(video_path, ffmpeg_path)
        
        if video_has_a:
            # Mix original audio with looped BGM audio
            # BGM is input 1, original video is input 0
            # original audio is at full volume (1.0) and faded in/out (1s)
            # BGM is at 0.15 volume and faded in/out (1s)
            filter_complex = (
                f"[0:a]afade=t=in:st=0:d=1,afade=t=out:st={duration-1}:d=1[orig];"
                f"[1:a]atrim=0:{duration},asetpts=PTS-STARTPTS,volume=0.15,afade=t=in:st=0:d=1,afade=t=out:st={duration-1}:d=1[bgm];"
                f"[orig][bgm]amix=inputs=2:duration=first[mixed]"
            )
            cmd = [
                ffmpeg_path, "-y",
                "-i", video_path,
            ...
            # Video has no audio. Just loop BGM and overlay it.
            filter_complex = (
                f"[1:a]atrim=0:{duration},asetpts=PTS-STARTPTS,volume=0.15,afade=t=in:st=0:d=1,afade=t=out:st={duration-1}:d=1[bgm]"
            )
            cmd = [
        ...
        print(f"Running mixing command: {' '.join(cmd)}")
        res = subprocess.run(cmd, capture_output=True, text=True, check=True)
        
        # Replace the original file with the mixed file
        if os.path.exists(temp_output):
            os.remove(video_path)
            os.rename(temp_output, video_path)
            print("Background music mixed successfully!")
    except Exception as e:
        print(f"Warning: Failed to mix background music. Error: {e}")
        if 'res' in locals():
            print(f"FFmpeg stdout: {res.stdout}\nFFmpeg stderr: {res.stderr}")
        if os.path.exists(temp_output):
            try:
                os.remove(temp_output)
            except Exception:
...
def slice_video(video_path, start_sec, end_sec, output_path, ffmpeg_path, watermark="", orientation="16:9", is_podcast=False, speaker_timeline=None, engine_mode="standard", transcript_path=None, title="", intro_hook="", burn_subtitles=True, bgm_path=None):
    ...
    if engine_mode in ["opsi_a", "opsi_b"]:
        ...
        if res.returncode != 0:
            ...
            if bgm_path:
                mix_bgm_audio(output_path, bgm_path, ffmpeg_path, end_sec - start_sec)
            return output_path
